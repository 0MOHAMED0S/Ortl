...
@section('styles')
    ...
    <style>
        ...
        .student-avatar {
            ...
            background-color: #f0f2f5; color: #2d8a74;
            display: flex; align-items: center; justify-content: center;
            font-weight: bold; flex-shrink: 0; border: 1px solid #e2e8f0;
        }
        ...
        /* --- Smart Search Styles --- */
        .search-box { position: relative; min-width: 350px; flex-grow: 1; }
        .search-icon-wrapper {
            position: absolute; right: 15px; top: 50%; transform: translateY(-50%);
            color: #94a3b8; background: none; border: none; outline: none; padding: 0; transition: 0.3s;
        }
        .search-icon-wrapper:hover { color: #2d8a74; cursor: pointer; }
        .search-box input {
            padding-right: 40px; border-radius: 12px; border: 1px solid #e2e8f0;
            padding-top: 10px; padding-bottom: 10px; transition: 0.3s; width: 100%;
        }
        ...
        .custom-input { border: 1px solid #e2e8f0; border-radius: 10px !important; padding: 10px 15px; font-size: 0.95rem; background-color: #f8fafc; transition: all 0.3s ease; }
        ...
        .custom-toast {
            ...
            backdrop-filter: blur(10px); border-radius: 12px; margin-bottom: 15px;
        ...
        .custom-toast.success { border-left-color: #2d8a74; }
        .custom-toast.error { border-left-color: #e11d48; }
        .toast-content { padding: 15px 18px; display: flex; align-items: center; gap: 12px; }
        .toast-icon { font-size: 1.6rem; }
        .success .toast-icon { color: #2d8a74; }
        .error .toast-icon { color: #e11d48; }
        .toast-body { flex-grow: 1; text-align: right; }
        .toast-title { display: block; font-weight: 800; font-size: 0.9rem; color: #1e293b; }
        .toast-message { margin: 0; font-size: 0.8rem; color: #64748b; }
        .toast-close { background: none; border: none; font-size: 1.2rem; color: #94a3b8; cursor: pointer; order: -1; }
        ...
        .success .toast-progress::before { background: #2d8a74; animation: progressRun 5s linear forwards; }
        ...
        .filter-badge {
            padding: 6px 16px; border-radius: 50px; background: #fff; border: 1px solid #e2e8f0;
            font-size: 0.85rem; font-weight: 600; color: #64748b; cursor: pointer; transition: all 0.2s; white-space: nowrap;
        }
        .filter-badge.active { background: #2d8a74; color: #fff; border-color: #2d8a74; box-shadow: 0 4px 10px rgba(45, 138, 116, 0.2); }
        .filter-badge:hover:not(.active) { border-color: #2d8a74; color: #2d8a74; }

        /* --- Creative Gift Modal Styles --- */
        .gift-card-header {
            background: linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%);
            border-radius: 20px 20px 0 0; padding: 30px; text-align: center;
            position: relative; overflow: hidden;
        }
        .gift-icon-anim { font-size: 3.5rem; color: #fff; text-shadow: 0 4px 10px rgba(0,0,0,0.1); animation: bounce 2s infinite; }
        @keyframes bounce { 0%, 20%, 50%, 80%, 100% {transform: translateY(0);} 40% {transform: translateY(-10px);} 60% {transform: translateY(-5px);} }
        .gift-selection-box { border: 2px dashed #e2e8f0; border-radius: 15px; transition: all 0.3s; }
        .gift-selection-box:focus-within { border-color: #ff9a9e; background-color: #fffafb; }
        ...
        .profile-avatar-wrapper img, .profile-avatar-wrapper .student-avatar-large {
        ...
        .info-list-item {
            ...
            padding: 10px; border-radius: 10px; background: #f8fafc; transition: 0.2s;
        }
        .info-list-item:hover { background: #f1f5f9; }
        .info-list-icon { width: 32px; height: 32px; border-radius: 8px; background: white; color: #2d8a74; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 5px rgba(0,0,0,0.02); }
        .info-list-content h6 { font-size: 0.75rem; color: #64748b; margin: 0 0 2px 0; text-transform: uppercase; }
        .info-list-content p { font-size: 0.95rem; color: #1e293b; font-weight: 600; margin: 0; word-break: break-word; }

        .package-scroll { max-height: 250px; overflow-y: auto; scrollbar-width: thin; }
        .package-scroll::-webkit-scrollbar { width: 4px; }
        .package-scroll::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
        ...
        @media (max-width: 768px) {
            ...
            .filter-container-wrapper input[type="date"] { width: 100% !important; margin-bottom: 10px; }
            .fixed-alert-container { right: 10px; left: 10px; top: 10px; width: auto; max-width: none; }
        }

        .bg-soft-primary { background-color: rgba(45, 138, 116, 0.1); }
        .transition-hover { transition: all 0.3s ease; }
        .transition-hover:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0,0,0,0.05) !important; border-color: #2d8a74 !important; }
        .stat-icon { width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; }
        .badge-new { background: linear-gradient(45deg, #ff5f6d, #ffc371); color: white; font-size: 0.6rem; padding: 2px 8px; border-radius: 10px; margin-right: 5px; animation: pulse 1.5s infinite; }
...
<div class="container-fluid p-3 p-md-4">
    ...
    <div class="row g-3 mb-4 text-center text-md-start">
        <div class="col-12 col-md-6 col-lg-3">
            <div class="p-3 bg-white rounded-3 shadow-sm d-flex align-items-center gap-3 border h-100 transition-hover">
                <div class="stat-icon bg-primary bg-opacity-10 text-primary p-2 rounded-circle"><i class="fa-solid fa-user-graduate fs-5"></i></div>
                <div><h4 class="m-0 fw-bold fs-5">{{ number_format($totalStudents) }}</h4><small class="text-muted fw-bold">إجمالي الطلاب</small></div>
            </div>
        </div>
        <div class="col-12 col-md-6 col-lg-3">
            <div class="p-3 bg-white rounded-3 shadow-sm d-flex align-items-center gap-3 border h-100 transition-hover">
                <div class="stat-icon bg-warning bg-opacity-10 text-warning p-2 rounded-circle"><i class="fa-solid fa-hourglass-half fs-5"></i></div>
                <div><h4 class="m-0 fw-bold fs-5">{{ number_format($totalMinutesRegistered) }}</h4><small class="text-muted fw-bold">دقائق الرصيد الكلية</small></div>
            </div>
        </div>
        <div class="col-12 col-md-6 col-lg-3">
            <div class="p-3 bg-white rounded-3 shadow-sm d-flex align-items-center gap-3 border h-100 transition-hover">
                <div class="stat-icon bg-danger bg-opacity-10 text-danger p-2 rounded-circle"><i class="fa-solid fa-gift fs-5"></i></div>
                <div><h4 class="m-0 fw-bold fs-5">{{ number_format($totalGifts) }}</h4><small class="text-muted fw-bold">إجمالي الهدايا</small></div>
            </div>
        </div>
        <div class="col-12 col-md-6 col-lg-3">
            <div class="p-3 bg-white rounded-3 shadow-sm d-flex align-items-center gap-3 border h-100 transition-hover">
                <div class="stat-icon bg-success bg-opacity-10 text-success p-2 rounded-circle"><i class="fa-solid fa-box-open fs-5"></i></div>
                <div><h4 class="m-0 fw-bold fs-5">{{ number_format($activePackagesCount) }}</h4><small class="text-muted fw-bold">باقات نشطة حالياً</small></div>
            </div>
        </div>
    </div>
    ...
    <form action="{{ url()->current() }}" method="GET" id="searchFilterForm">
        ...
        <div class="filter-scroll-wrapper mb-4" id="filterButtons">
            <button type="button" class="filter-badge {{ request('filter', 'all') == 'all' ? 'active' : '' }}" onclick="submitFilter('all')">الكل</button>
            <button type="button" class="filter-badge {{ request('filter') == 'gift' ? 'active' : '' }}" onclick="submitFilter('gift')">باقات هدايا 🎁</button>
            @foreach($allPackages as $pkg)
                <button type="button" class="filter-badge {{ request('filter') == 'pkg-'.$pkg->name ? 'active' : '' }}" onclick="submitFilter('pkg-{{ $pkg->name }}')">{{ $pkg->name }}</button>
            @endforeach
        </div>
        ...
        <div class="filter-container filter-container-wrapper">
            <div class="d-flex align-items-center gap-2">
                ...
                <span class="badge bg-soft-primary text-primary rounded-pill px-3" id="resultsCount">{{ method_exists($students, 'total') ? $students->total() : $students->count() }} نتيجة لـ بحثك</span>
            </div>

            <div class="d-flex align-items-center gap-2 flex-wrap flex-md-nowrap w-100 w-md-auto">
                <select name="country" id="countryFilter" class="form-select form-select-sm rounded-pill border-0 shadow-sm px-3 py-2" style="width: 150px;" onchange="document.getElementById('searchFilterForm').submit()">
                    <option value="all" {{ request('country', 'all') == 'all' ? 'selected' : '' }}>كل الدول</option>
                    @foreach($countries as $c)
                        <option value="{{ $c->name }}" {{ request('country') == $c->name ? 'selected' : '' }}>{{ $c->name }}</option>
                    @endforeach
                </select>

                <input type="date" name="date" id="joinDateFilter" class="form-control form-control-sm rounded-pill border-0 shadow-sm px-3 py-2" style="width: 160px;" value="{{ request('date') }}" onchange="document.getElementById('searchFilterForm').submit()">

                <div class="search-box flex-grow-1">
                    <button type="submit" class="search-icon-wrapper" id="searchIcon" title="بحث">
                        <i class="fa-solid fa-search"></i>
                    </button>
                    <input type="text" name="search" id="searchInput" class="form-control rounded-pill bg-white" placeholder="ابحث بالاسم، الهاتف، الإيميل واضغط Enter..." value="{{ request('search') }}">
                    ...
                    <tbody id="studentsTableBody">
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">
                                ...
                                <a href="{{ route('admin.students') }}" class="btn btn-outline-primary btn-sm rounded-pill mt-2">عرض كل الطلاب</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="col-12 col-xl-5">
            <div class="stats-summary-card h-100 shadow-lg border-0">
                <div class="d-flex justify-content-between align-items-start mb-4">
                    ...
                    <div class="bg-white bg-opacity-20 p-2 rounded-3 text-white"><i class="fa-solid fa-chart-line fs-4"></i></div>
                </div>
                <div class="display-stats py-3">
                    <h1 class="fw-bold text-white mb-2" id="filteredStudentsDisplay">{{ number_format($totalStudents) }} <small class="fs-4">طالب</small></h1>
                    <span class="badge bg-white bg-opacity-25 rounded-pill text-white border-0 py-2 px-3" id="statsDateLabel">الفترة: كافة الأوقات</span>
                </div>
            </div>
        </div>
            ...
            <div class="stats-filter-box h-100">
                ...
                <div class="row g-3">
                    <div class="col-12 col-md-6">
                        ...
                        <input type="date" id="statsFromDate" class="form-control custom-input" onchange="calculatePeriodStats()">
                    </div>
                    <div class="col-12 col-md-6">
                        <label class="form-label fw-bold">إلى تاريخ</label>
                        <input type="date" id="statsToDate" class="form-control custom-input" onchange="calculatePeriodStats()">
                    </div>
                    <div class="col-12 mt-4">
                        <label class="form-label fw-bold d-block mb-3">اختصارات زمنية</label>
                        <div class="filter-scroll-wrapper">
                            <button class="btn btn-light rounded-pill px-4 fw-bold small text-muted border quick-stat-btn" onclick="quickStatsFilter('today', this)">اليوم</button>
                            <button class="btn btn-light rounded-pill px-4 fw-bold small text-muted border quick-stat-btn" onclick="quickStatsFilter('month', this)">هذا الشهر</button>
                            <button class="btn btn-light rounded-pill px-4 fw-bold small text-muted border quick-stat-btn" onclick="quickStatsFilter('lastMonth', this)">الشهر الماضي</button>
                            <button class="btn btn-light rounded-pill px-4 fw-bold small text-muted border quick-stat-btn active-all" onclick="quickStatsFilter('all', this)">كل الأوقات</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-0">
                    <div class="col-12 col-md-5 bg-white border-end position-relative">
                        ...
                        <div class="text-center px-4 pb-4">
                            ...
                            <span id="modalCountry" class="badge bg-light text-dark border px-3 py-1 rounded-pill mb-4"><i class="fa-solid fa-location-dot text-danger me-1"></i> </span>
                    ...
                    <div class="col-12 col-md-7 p-4 p-md-5 bg-light bg-opacity-50">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h6 class="fw-bold m-0 text-dark"><i class="fa-solid fa-box-open text-primary me-2"></i>سجل الاشتراكات</h6>
                            <span class="badge bg-white text-primary border px-3 py-2 rounded-pill shadow-sm" id="packageCount"></span>
                        </div>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
            <form id="updateForm" method="POST" action="">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="form-label"><i class="fa-regular fa-user text-muted me-1"></i>الاسم بالكامل</label>
                            <input type="text" name="name" id="edit_name" class="form-control custom-input" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label"><i class="fa-solid fa-phone text-muted me-1"></i>رقم الهاتف</label>
                            <input type="text" name="phone" id="edit_phone" class="form-control custom-input" dir="ltr">
                        </div>
                        <div class="col-12">
                            <label class="form-label"><i class="fa-solid fa-graduation-cap text-muted me-1"></i>المؤهل العلمي</label>
                            <input type="text" name="qualification" id="edit_qualification" class="form-control custom-input">
                        </div>
                        <div class="col-12 col-md-6">
                            <label class="form-label"><i class="fa-solid fa-briefcase text-muted me-1"></i>الحالة المهنية</label>
                            <input type="text" name="professional_status" id="edit_job" class="form-control custom-input">
                        </div>
                        <div class="col-12 col-md-6">
                            <label class="form-label"><i class="fa-solid fa-venus-mars text-muted me-1"></i>الجنس</label>
                ...
                <div class="modal-footer border-0 p-4 pt-0">
                    <button type="button" class="btn btn-light rounded-pill px-4 fw-bold text-muted flex-grow-1 flex-md-grow-0" data-bs-dismiss="modal">إلغاء</button>
                    <button type="submit" class="btn btn-primary rounded-pill px-5 fw-bold shadow flex-grow-1 flex-md-grow-0">حفظ التعديلات</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form id="giftForm" method="POST" action="">
                ...
                <div class="modal-body p-4 text-center">
                    ...
                    <div class="gift-selection-box p-3 text-start bg-light">
                        <label class="form-label fw-bold small text-primary mb-2"><i class="fa-solid fa-star me-1"></i> اختر الباقة المميزة</label>
                        <select name="package_id" class="form-select custom-input fw-bold bg-white" required>
                            ...
                            @isset($allPackages) @foreach($allPackages as $pkg) <option value="{{ $pkg->id }}">🎁 {{ $pkg->name }} — ({{ $pkg->base_minutes + $pkg->bonus_minutes }} دقيقة)</option> @endforeach @endisset
                        </select>
                    </div>
                </div>
                <div class="modal-footer border-0 pb-4 justify-content-center">
                    <button type="submit" class="btn btn-lg rounded-pill px-5 fw-bold shadow-sm w-100 mx-3" style="background: linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%); color: white; border: none;">إرسال الهدية الآن</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function quickStatsFilter(period, btn) {
        ...
        if (period === 'all') {
            document.getElementById('filteredStudentsDisplay').innerHTML = '{{ number_format($totalStudents) }} <small class="fs-4">طالب</small>';
        ...
        let fromStr = todayStr, toStr = todayStr;
    ...
    function showStudentDetails(student) {
        ...
        document.getElementById('modalCountry').innerHTML = `<i class="fa-solid fa-location-dot text-danger me-1"></i> ${student.country_name}`;
        ...
        if (student.all_packages && student.all_packages.length > 0) {
            ...
            student.all_packages.forEach(pkg => {
                const giftTag = pkg.is_gift ? `<span class="badge bg-warning bg-opacity-10 text-warning ms-2 rounded-pill" style="font-size: 0.6rem;">هدية 🎁</span>` : '';
                tableBody.innerHTML += `
            ...
            tableBody.innerHTML = '<tr><td colspan="3" class="text-center py-5 text-muted"><i class="fa-solid fa-box-open fs-2 mb-2 opacity-50"></i><br>لا توجد باقات حالية</td></tr>';
        }
